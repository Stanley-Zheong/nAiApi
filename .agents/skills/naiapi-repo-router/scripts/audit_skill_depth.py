...
def audit(repo: Path, router: str, skills_root_arg: str) -> list[str]:
    problems: list[str] = []
    repo = repo.resolve()
    skills_root = (repo / skills_root_arg).resolve()
    try:
        skills_root.relative_to(repo)
    except ValueError:
        problems.append(f"skills root escapes repository root: {skills_root_arg}")
        return problems
    ...
    for package in sorted(actual_packages):
        text_path = skills_root / package / "SKILL.md"
...
def main() -> int:
    ...
    problems = audit(args.repo, args.router, args.skills_root)
