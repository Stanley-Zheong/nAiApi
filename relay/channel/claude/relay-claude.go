...
// createClaudeFileSource builds a file source for an inline file content part.
// The MIME type is derived from the filename extension so that downstream
// handling can distinguish documents/text from unsupported binary blobs.
...
// buildClaudeFileMessage converts an inline file content part into a Claude
// media block. PDFs become document blocks, text files are decoded into text
// blocks, and any other (unsupported) file type is skipped (returns nil, nil).
